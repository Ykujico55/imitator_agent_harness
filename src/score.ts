...
import { isCodeFile, isTestPath } from "./evidence-path.ts";
...
export function assessRepository(repo: RepositoryProfile, task: TaskSpec, config: HarnessConfig, now = new Date()): RepositoryAssessment {
  ...
  const domain = domainMetadataSignal(`${repo.fullName} ${repo.description} ${repo.topics.join(" ")}`, task);

  const maturityReasons: string[] = [];
  let maturity = Math.min(38, Math.log10(repo.stars + 1) * 11) + Math.min(12, Math.log10(repo.forks + 1) * 5);
  if (files.some((path) => isTestPath(path) && isCodeFile(path))) { maturity += 18; maturityReasons.push("Behavioral test files"); }
  ...
  // Preserve the prior allowlisted-candidate baseline without treating legal
  // metadata as evidence for or against technical adaptability.
  const transferReasons: string[] = ["license-independent-transfer-baseline: 65; structural heuristic only"];
  let transfer = 65;
  ...
  if (files.some((path) => isTestPath(path) && isCodeFile(path))) { design += 20; designReasons.push("Design is exercised by tests"); }
  ...
  const dimensions = {
    domainMatch: dimension(domain.score, domain.reasons),
  ...
  if (!domain.matched.length) rejectionReasons.push("domain-metadata-missing: same-domain reference required");
  ...
  const license = assessLicense(repo.license, config);
  rejectionReasons.push(...license.rejectionReasons);
  return { repository: repo, licenseWarnings: license.warnings, dimensions, overall, accepted: rejectionReasons.length === 0, rejectionReasons };
}
